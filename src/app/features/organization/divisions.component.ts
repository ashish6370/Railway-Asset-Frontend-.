  ...
  template: `
    <div class="page-header">
      <div class="header-content">
        ...
        <p class="page-subtitle">Manage Divisions</p>
      </div>
      <div class="header-actions">
        <button mat-flat-button class="action-btn" (click)="toggleForm()">
          <mat-icon>{{ showForm ? 'close' : 'add' }}</mat-icon> {{ showForm ? 'Cancel' : 'New Division' }}
        </button>
      </div>
    </div>

    <mat-card class="glass-card" *ngIf="showForm" style="margin-bottom: 20px; padding: 20px;">
      <h3>{{ editingId ? 'Edit' : 'Add' }} Division</h3>
      <div class="form-grid">
        ...
        <table mat-table [dataSource]="data" class="custom-table" style="width: 100%;">
          ...
          <tr class="mat-row" *matNoDataRow>
            <td class="mat-cell" colspan="4" style="text-align: center; padding: 20px;">
              No data available. Add a new division to get started.
            </td>
          </tr>
        </table>
      </div>
...
export class DivisionsComponent implements OnInit {
  ...
  save() {
    if (this.editingId) {
      this.api.updateDivision(this.editingId, this.formData).subscribe({
        next: () => {
      ...
      this.api.addDivision(this.formData).subscribe({
        next: () => {
  ...
  delete(id: number) {
    if (confirm('Are you sure you want to delete this record?')) {
      this.api.deleteDivision(id).subscribe({
        next: () => {
