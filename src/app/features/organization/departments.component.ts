    ...
    <div class="page-header">
      ...
      <div class="header-actions">
        <button mat-flat-button class="action-btn" (click)="toggleForm()">
          <mat-icon>{{ showForm ? 'close' : 'add' }}</mat-icon> {{ showForm ? 'Cancel' : 'New Department' }}
        </button>
      </div>
    </div>
        ...
        <table mat-table [dataSource]="data" class="custom-table" style="width: 100%;">
          ...
          <tr class="mat-row" *matNoDataRow>
            <td class="mat-cell" colspan="4" style="text-align: center; padding: 20px;">
              No data available. Add a new department to get started.
            </td>
          </tr>
        </table>
      </div>
...
export class DepartmentsComponent implements OnInit {
  ...
  save() {
    if (this.editingId) {
      this.api.updateDepartment(this.editingId, this.formData).subscribe({
        next: () => {
      ...
      this.api.addDepartment(this.formData).subscribe({
        next: () => {
